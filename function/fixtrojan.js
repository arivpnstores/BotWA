...
module.exports = {
  trigger: ['fixtrojan', 'resettrojan', 'repairrojan'],
  execute: async (sock, m, args, { isOwner } = {}) => {
    ...
    if (!serverId || !username) {
      return sock.sendMessage(
        chat,
        { text: 'Format salah.\nGunakan: .fixtrojan <server> <username>\nContoh: .fixtrojan 1 trialari' },
        { quoted: m }
      );
    }
    ...
    // ===== ENDPOINT FIXTROJAN (LOCK -> UNLOCK) =====
    // Kalau endpoint di panel kamu beda, cukup ganti 2 baris ini:
    const lockUrl = `https://${srv.domain}/vps/locktrojan/${encodeURIComponent(username)}`;
    const unlockUrl = `https://${srv.domain}/vps/unlocktrojan/${encodeURIComponent(username)}`;
    ...
    try {
      ...
      if (lockJson?.meta?.code !== 200 || !lockJson?.data) {
        ...
        return sock.sendMessage(chat, { text: `❌ Gagal LOCK TROJAN` }, { quoted: m });
      }
      ...
      const msg = `✅ FIX TROJAN SUCCESS (LOCK → UNLOCK)
----------------------------
...
*© WhatsApp Bots - 2026*`;
      ...
      console.error('❌ FIXTROJAN ERROR:', e?.message || e);
      await sock.sendMessage(
        chat,
        { text: `❌ Gagal proses FIX TROJAN: 'Terjadi kesalahan saat menghubungi server'}` },
        { quoted: m }
      );
    }
  }
