...
module.exports = {
  trigger: ['fixvmess', 'resetvmess', 'repairvmess'],
  execute: async (sock, m, args, { isOwner } = {}) => {
    ...
    if (!serverId || !username) {
      return sock.sendMessage(
        chat,
        { text: 'Format salah.\nGunakan: .fixvmess <server> <username>\nContoh: .fixvmess 1 trialari' },
        { quoted: m }
      );
    }
    ...
    // ===== ENDPOINT FIXVMESS (LOCK -> UNLOCK) =====
    // sesuai contoh bot lama:
    // LOCK  : PATCH /vps/lockvmess/{username}
    // UNLOCK: PATCH /vps/unlockvmess/{username}
    const lockUrl = `https://${srv.domain}/vps/lockvmess/${encodeURIComponent(username)}`;
    const unlockUrl = `https://${srv.domain}/vps/unlockvmess/${encodeURIComponent(username)}`;
    ...
    try {
      ...
      if (lockJson?.meta?.code !== 200 || !lockJson?.data) {
        ...
        return sock.sendMessage(chat, { text: `❌ Gagal LOCK VMESS` }, { quoted: m });
      }

      // jeda dikit biar server sempat apply (opsional)
      ...
      const msg = `✅ FIX VMESS SUCCESS (LOCK → UNLOCK)
----------------------------
...
*© WhatsApp Bots - 2026*`;
      ...
      console.error('❌ FIXVMESS ERROR:', e?.message || e);
      await sock.sendMessage(
        chat,
        { text: `❌ Gagal proses FIX VMESS: 'Terjadi kesalahan saat menghubungi server'}` },
        { quoted: m }
      );
    }
  }
