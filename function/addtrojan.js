...
module.exports = {
  trigger: ['addtrojan', 'buattrojan'],
  execute: async (sock, m, args, { isOwner }) => {
    const chat = m.key.remoteJid;

    // format BARU: .addtrojan <server> <username> <jumlah_hari>
    ...
    if (!serverId || !username || isNaN(days)) {
      return sock.sendMessage(
        chat,
        { text: 'Format salah.\nGunakan: .addtrojan <server> <username> <jumlah_hari>\nContoh: .addtrojan 1 usertrojan 30' },
        { quoted: m }
      );
    }
    ...
    if (!srv) {
      return sock.sendMessage(
        chat,
        { text: '❌ Server yang anda pilih tidak ada. Gunakan ID sesuai daftar di config_servers.js.' },
        { quoted: m }
      );
    }

    // ===== CEK IZIN OWNER / OWNER SERVER =====
    let allowed = isOwner;
    try {
    ...
    // ===== API CREATE TROJAN =====
    const apiUrl = `http://${srv.domain}/vps/trojanall`;
    ...
    try {
      ...
      if (!meta || meta.code !== 200 || !d) {
        const errMsg = res.data?.message || meta?.message || 'Gagal membuat akun TROJAN';
        return sock.sendMessage(chat, { text: `❌ Respons error` }, { quoted: m });
      }

      // ===== PESAN SUKSES =====
      const msg = `Trojan Account Creation Success
----------------------------
Username       : ${d.username}
Host           : ${d.hostname}
Port TLS       : ${d.port?.tls || '-'}
...
*© WhatsApp Bots - 2026*`;
      ...
      await sock.sendMessage(chat, { text: `❌ Gagal create TROJAN` }, { quoted: m });
    }
  }
};
