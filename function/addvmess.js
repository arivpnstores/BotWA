  ...
  execute: async (sock, m, args, { isOwner }) => {
    ...
    if (!serverId || !username || isNaN(days)) {
      return sock.sendMessage(
        chat,
        { text: 'Format salah.\nGunakan: .addvmess <server> <username> <jumlah_hari>\nContoh: .addvmess 1 zuwita 30' },
        { quoted: m }
      );
    }
    ...
    const apiUrl = `http://${srv.domain}/vps/vmessall`;
    ...
    try {
      ...
      if (!meta || meta.code !== 200 || !d) {
        const errMsg = res.data?.message || meta?.message || 'Gagal membuat akun VMESS';
        return sock.sendMessage(chat, { text: `❌ Respons error` }, { quoted: m });
      }

      const msg = `VMess Account Creation Success
----------------------------
Username       : ${d.username}
Host           : ${d.hostname}
Port TLS       : ${d.port?.tls || '-'}
...
*© WhatsApp Bots - 2026*`;
      ...
      await sock.sendMessage(chat, { text: `❌ Gagal create VMESS` }, { quoted: m });
    }
  }
};
