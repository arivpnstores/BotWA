...
module.exports = {
  trigger: ['deletetrojan', 'deltrojan'],
  execute: async (sock, m, args, { isOwner } = {}) => {
    ...
    try {
      ...
      // format: .deltrojan <serverId> <username>
      ...
      if (!serverId || !username) {
        return sock.sendMessage(
          chat,
          {
            text:
              'Format salah.\n' +
              'Gunakan: .deltrojan <serverId> <username>\n' +
              'Contoh: .deltrojan 1 usertrojan'
          },
          { quoted: m }
        );
      }
      ...
      // Endpoint delete TROJAN
      const web_URL = `http://${srv.domain}/vps/deletetrojan`;
...
-H "Authorization: ${AUTH_TOKEN}"`;
      ...
      exec(curlCommand, async (err, stdout) => {
        if (err || !stdout) {
          console.log('[DELETE TROJAN CURL ERROR]', err?.message);
        ...
        try {
          ...
          console.log('[DELETE TROJAN PARSE ERROR]', stdout);
        ...
        if (d.meta?.code !== 200 && d.status !== 'success') {
          return sock.sendMessage(
            chat,
            { text: `❌ ${d.message || d.meta?.message || 'Gagal delete Trojan'}` },
            { quoted: m }
          );
        }
...
*© WhatsApp Bots - 2026*`;
      ...
      console.log('[DELETE TROJAN ERROR]', e.message);
      return sock.sendMessage(chat, { text: '❌ Terjadi kesalahan saat delete Trojan' }, { quoted: m });
    }
  },
};
