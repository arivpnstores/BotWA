...
module.exports = {
  trigger: ['fixvless', 'resetvless', 'repairvless'],
  execute: async (sock, m, args, { isOwner } = {}) => {
    ...
    if (!serverId || !username) {
      return sock.sendMessage(
        chat,
        { text: 'Format salah.\nGunakan: .fixvless <server> <username>\nContoh: .fixvless 1 trialari' },
        { quoted: m }
      );
    }
    ...
    // ===== ENDPOINT FIXVLESS (LOCK -> UNLOCK) =====
    // Sesuaikan kalau endpoint di server kamu beda:
    // LOCK  : PATCH /vps/lockvless/{username}
    // UNLOCK: PATCH /vps/unlockvless/{username}
    const lockUrl = `https://${srv.domain}/vps/lockvless/${encodeURIComponent(username)}`;
    const unlockUrl = `https://${srv.domain}/vps/unlockvless/${encodeURIComponent(username)}`;

    const lockCmd = `curl -sS -X PATCH "${lockUrl}" -H "accept: application/json" -H "Content-Type: application/json" -H "Authorization: Bearer ${srv.token}" --data '{}'`;
    const unlockCmd = `curl -sS -X PATCH "${unlockUrl}" -H "accept: application/json" -H "Content-Type: application/json" -H "Authorization: Bearer ${srv.token}" --data '{}'`;

    await sock.sendMessage(
      chat,
      { text: `🔧 Fix VLESS: *${username}*\nServer: *${serverId}*\n⏳ Proses: LOCK → UNLOCK...` },
      { quoted: m }
    );
    ...
    try {
      ...
      if (lockJson?.meta?.code !== 200 || !lockJson?.data) {
        ...
        return sock.sendMessage(chat, { text: `❌ Gagal LOCK VLESS` }, { quoted: m });
      }

      // jeda sedikit biar server apply (opsional)
      ...
      const msg = `✅ FIX VLESS SUCCESS (LOCK → UNLOCK)
----------------------------
...
*© WhatsApp Bots - 2026*`;
      ...
      console.error('❌ FIXVLESS ERROR:', e?.message || e);
      await sock.sendMessage(
        chat,
        { text: `❌ Gagal proses FIX VLESS: 'Terjadi kesalahan saat menghubungi server'}` },
        { quoted: m }
      );
    }
  }
