...
module.exports = {
  trigger: ['trialtrojan', 'buattrialtrojan', 'trojantrial'],
  execute: async (sock, m, args, { isOwner } = {}) => {
    ...
    if (!serverId) {
      return sock.sendMessage(
        chat,
        { text: 'Format salah. Gunakan: .trialtrojan <server>\nContoh: .trialtrojan 1' },
        { quoted: m }
      );
    }
    ...
    // ===== endpoint trial trojan =====
    // ganti jika endpoint kamu berbeda
    const apiUrl = 'http://' + srv.domain + '/vps/trialtrojanall';

    // ===== payload trial (tanpa expired/hari) =====
    const payload = {
      timelimit: '3h'
    };
    ...
    try {
      ...
      if (!meta || meta.code !== 200 || !d) {
        const errMsg = res.data?.message || meta?.message || 'Gagal membuat akun TRIAL TROJAN';
        return sock.sendMessage(chat, { text: `❌ Respons error` }, { quoted: m });
      }

      const msg = `✅ TRIAL Trojan Account Creation Success
----------------------------
Username       : ${d.username}
Host           : ${d.hostname}
Port TLS       : ${d.port?.tls ?? '-'}
...
*© WhatsApp Bots - 2026*`;
      ...
      await sock.sendMessage(
        chat,
        { text: `❌ Gagal create TRIAL TROJAN` },
        { quoted: m }
      );
    }
  }
